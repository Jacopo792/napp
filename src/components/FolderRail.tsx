        ...
        <div className="overflow-hidden">
          ...
          {adding ? (
            ...
            <button
              ...
              className="mobile-folder-row flex min-h-12 w-full items-center gap-3 px-4 text-left text-accent"
            >
              <Plus size={16} strokeWidth={2} className="shrink-0" />
              <span className="text-[15px] font-medium">New folder</span>
            </button>
          )}
        </div>

        <p className="label px-5 pt-5 pb-2 text-ink-4">Tags</p>

        <div className="mb-4 overflow-hidden">
          {meta.tags.map((tag) => {
            const palette = TAG_COLORS.find((color) => color.id === tag.color) ?? TAG_COLORS[0];
            const swatch = palette.darkFg;
            ...
            return (
              <div
                ...
                className={`mobile-folder-row flex min-h-12 items-center gap-3 px-4 ${active ? "bg-accent-wash" : ""}`}
              >
                <span
                  aria-hidden
                ...
                />
                <span
                  className={`min-w-0 flex-1 truncate text-[15px] ${active ? "text-accent" : "text-ink-2"}`}
                >
                  {tag.name}
                </span>
                <button
                  ...
                  className="icon-button flex h-8 w-8 shrink-0 items-center justify-center text-ink-4 hover:text-danger"
                >
          ...
          <div className="mobile-folder-row flex min-h-12 items-center gap-3 px-4">
            <Tag size={16} strokeWidth={1.9} className="shrink-0 text-accent" />
            <input
              value={newTagName}
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="mx-2 flex h-9 items-center gap-2 rounded-lg px-3 hover:bg-page">
          <span
            ...
            className="h-2.5 w-2.5 shrink-0 rounded-full"
            style={{
              background: (TAG_COLORS.find((c) => c.id === newTagColor) ?? TAG_COLORS[0]).darkFg,
            }}
          />
          <input
            value={newTagName}
