...
export function useCollaborativeNote(
  ...
  if (!noteId || !identity) {
    return { doc: null, provider: null, ready: false, cached: false, connection: "connecting", refusal: "", waking: false };
  }
  const doc = documents.get(noteId) ?? null;
  return { doc, provider: null, ready: Boolean(doc), cached: Boolean(doc), connection: "connected", refusal: "", waking: false };
}
